/**
 * showMessage({ message:".....要显示的信息", isSuccess:false, callback:function(){
 * //window.close(); } });
 * 
 * @param message
 *            要提示的信息
...
function showMessage(messageOrOptions, _isSuccess) {
	...
	if (!defaultOpts.message) {
		defaultOpts.message = defaultOpts.isSuccess === true ? '成功，但没有提示信息！'
				: '失败，但没有提示信息！'
	}
	// 判断消息是否重复
	var ru = null;
	...
	if (defaultOpts.singleton) {
		...
		var $singletonInfo = $container.find(".info-singleton"
				+ (defaultOpts.messageClass ? "." + defaultOpts.messageClass
						: ""));
		$singletonInfo.length
				|| ($singletonInfo = $(
						'<div class="info-unit info-singleton '
								+ defaultOpts.messageClass
								+ '">'
								+ '<button class="close close-info"  type="button">×</button>'
								+ '<strong class="info-unit-message">提示信息</strong>'
								+ '</div>').appendTo($container));
		$singletonInfo.on('click', '.close-info', {
			unit : $singletonInfo
		}, _close_info);
		enrichInfo($singletonInfo);
		$infoUnit = $singletonInfo;
	} else {
		var $notSingletonInfo = $(
				'<div class="info-unit info-singleton-not  '
						+ defaultOpts.messageClass
						+ '">'
						+ '<button class="close close-info"   type="button">×</button>'
						+ '<strong class="info-unit-message">提示信息</strong>'
	...
	function enrichInfo($info) {
		...
		if (defaultOpts.autoClose === true)
			$info.slideDown(1000, function() {
				/*
				 * // 计算展示高度 var strongHeight = $strong.height(); var ht =
				 * strongHeight > 50 ? strongHeight : 50;
				 */
	...
	function repeatUnit() {
		...
		$container.find(
				'div.info-unit'
						+ (defaultOpts.messageClass ? "."
								+ defaultOpts.messageClass : "")).each(
				function() {
					__$unit = $(this);
					var $strong = __$unit.find(".info-unit-message");
					var __isRepeat = $strong.html() == __msg
							|| $strong.html() == __msgHtml;
					if (__isRepeat) {
